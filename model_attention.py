...
class DownStage(nn.Module):
    def __init__(self, in_ch, out_ch, emb_dim, num_blocks=2, downsample=True, use_attn=False, num_heads=4):
        super().__init__()
        self.blocks = nn.ModuleList()
        self.attns = nn.ModuleList() if use_attn else None
        
        ch = in_ch
        for _ in range(num_blocks):
            self.blocks.append(FiLMResBlock(ch, out_ch, emb_dim))
            if use_attn:
                self.attns.append(SelfAttention2d(out_ch, num_heads=num_heads))
            ch = out_ch
...
class UpStage(nn.Module):
    def __init__(self, in_ch, skip_ch, out_ch, emb_dim, num_blocks=2, upsample=True, use_attn=False, num_heads=4):
        super().__init__()
...
class ConditionalUNet(nn.Module):
    def __init__(
        ...
        for i, mult in enumerate(channel_mults):
            ...
            # Zapneme pozornost pro hlubší vrstvy (kde je mult >= 2)
            use_attn = (mult >= 2) 
            self.down_stages.append(DownStage(ch, out_ch, emb_dim, num_res_blocks, downsample=not is_last, use_attn=use_attn, num_heads=num_heads))
            ch = out_ch
            ch_list.append(ch)
        ...
        for i, mult in enumerate(reversed(channel_mults)):
            ...
            # Musíme zachovat symetrii: pozornost na stejných úrovních jako v DownStage
            # Klesáme zpět (mult jde např. 4 -> 2 -> 1)
            current_mult = channel_mults[-(i + 1)]
            use_attn = (current_mult >= 2)
            
            self.up_stages.append(UpStage(in_ch, skip_ch, out_ch, emb_dim, num_res_blocks, upsample=not is_first, use_attn=use_attn, num_heads=num_heads))
            ch = out_ch
