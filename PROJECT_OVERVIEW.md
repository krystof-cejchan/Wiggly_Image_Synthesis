...
**Handling arbitrary image sizes.** The network requires inputs divisible by 16, and real crops are arbitrary sizes, often much larger than the 128–384px range seen in training. `img2img.py` mirror-pads to a multiple of the stride, then processes the image through a **sliding window** (default 128px window, 64px stride) with overlapping windows blended together using a 2D Hann window (`create_blending_mask`) — this is what avoids visible seams between windows. (One implementation detail worth knowing: a plain Hann window is exactly zero at its own edges, which — before this was fixed — froze the outermost row/column of the canvas at its initial noise value; the blending mask now uses the *interior* of a slightly larger Hann window to keep every pixel's blend weight strictly positive.)

## 6. Interpolation vs. extrapolation — reaching pH outside 5.8–8.8

This is the more novel part of the project: the tooling doesn't just interpolate within the trained pH range, it also supports **requesting pH values the model never saw during training**, and does so via two different, independently-validated mechanisms depending on direction. `img2img.py`'s CLI dispatches to the correct one automatically (through `edit_to_pH`, defined in `img2img.py`) — the rest of this section is about *why* it needs to, not something you need to invoke separately.
...
The same warp machinery also handles requests **below** pH 5.8 during editing, by damping (rather than adding) waviness — shearing the existing traced centerline down to a lower target. This is not an optional simplification; it's necessary, for the reason noted at the end of §6.1: velocity extrapolation doesn't move the output under an img2img anchor, so editing below 5.8 needs the same geometric treatment as editing above 8.8. `img2img.edit_to_pH()` is the single entry point that looks at the requested pH and dispatches to the right mechanism automatically, in **either** direction — `img2img.py`'s `main()` calls it, so the plain CLI gets this correct without the caller needing to know any of the above. (It lives in `img2img.py`, not `ph_warp.py`, so that `img2img.py` can call it without a circular import — `ph_warp.py`'s lower-level pieces it depends on, `apply_ph_waviness`/`synth_displacement`/`warp_filament`, don't import `img2img` back.)

`sample.py` is the one place the *plain* velocity-extrapolation mechanism runs on its own with no geometric fallback available (there's no reference image to warp) — and it's correct there only for the below-5.8 direction; above-8.8 is wrong even in `sample.py`, per the finding above.
