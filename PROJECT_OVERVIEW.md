...
- `ph_warp.py` then synthesizes a transverse displacement sized to hit that extrapolated target waviness, and applies it to the *image itself* via `grid_sample` — warping only the traced fiber (inside a Gaussian envelope centered on it, so the background doesn't drag along and pick up unnatural warp artifacts), extending the frame by synthesized background where the warp needs more room than the original crop provides.
- The displacement is **broadband** (`bounded_broadband`), built in the Fourier domain so that its total RMS and the share of it landing in the 24–96px ripple band are set independently. That second number is the whole point: one frame-wide arc and ten small waves of the same amplitude have identical RMS, so a displacement that only controls amplitude produces the arc. Real crops hold the ripple share near 0.61 at every pH, and the synthesized displacement reproduces it.
- `refine_texture` is *not* run — see its docstring. Feeding any resampled image back through `edit_image` adds grainy salt-and-pepper texture across the whole frame at every strength from 0.05 up, because the model was trained only on real crops and never on its own output or on `grid_sample`-resampled ones.
...
**`img2img.py`'s CLI reaches the geometric warp automatically** — it calls `ph_warp.edit_to_pH()`, not `edit_image()`, so an out-of-range `--target_pH` is handled correctly in either direction with no special invocation. What the CLI does *not* do is rely on the network to extrapolate; that would be `--geometry_mode native`, which is wrong above 8.8 (produces the opposite of wavier) and largely ineffective below 5.8 (barely changes the output). `edit_image()` called directly performs only the velocity-extrapolation mechanism and no warp; use it to measure the model in isolation, not to produce results. (`sample.py`, which has no reference image to anchor anything, is the one place the plain velocity-extrapolation mechanism is actually correct on its own — and only for the below-5.8 direction; above-8.8 is wrong there too, per §6.2.)

**The warp is not reserved for out-of-range pH.** As of the ripple generation it is how *every* edit gets its shape, in-range included, because the model was measured to render a filament at only ~40% of a real crop's contrast whenever it has to draw that filament somewhere it must invent — 0.35–0.39 fiber depth against 0.86 for a real pH 8.8 crop, identically on the last two checkpoints, and no inference-side knob recovers it. Nulling the geometry conditioning, so the model only repaints texture around a filament that stays put, returns it to 0.98. So the model supplies texture and the warp supplies shape: the fiber that comes out is the source's own fiber, moved, which cannot lose contrast or break. Measured on a pH 5.8→8.8 edit, warp against native: 0.76 vs 0.35 fiber depth, 1.00 vs 0.93 continuity, 7.10 vs 8.49px total centreline rms (real 6.57), 4.24 vs 4.28px ripple (real 3.85).
